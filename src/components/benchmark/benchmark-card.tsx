...
import { Bookmark, CirclePlay, Clock3, ExternalLink, Eye, Heart, ThumbsUp, Trash2, TrendingUp, Users } from "lucide-react";
import { AccountAvatar } from "@/components/account-avatar";
import { PlatformBadge } from "@/components/accounts/platform-badge";
import { MiniSparkline } from "@/components/dashboard/mini-sparkline";
import type { BenchmarkListItem } from "@/lib/benchmark-accounts";
import { VIEW_PRIMARY_PLATFORMS } from "@/lib/providers/platform";

type BenchmarkCardProps = {
  account: BenchmarkListItem;
  isDeleting?: boolean;
  onDelete: (id: string) => void;
};

export function BenchmarkCard({ account, isDeleting = false, onDelete }: BenchmarkCardProps) {
  const detailHref = `/benchmark-accounts/${encodeURIComponent(account.id)}`;
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-1 pointer-events-auto">
              ...
              <button
                ...
                aria-label={`删除对标账号 ${account.handle}`}
              >
